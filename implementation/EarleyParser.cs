...
namespace PlanRecognitionExtension
{
    ...
    {
        public List<TaskType> AllTaskTypes { get; protected set; }
        public List<ActionType> AllActionTypes { get; protected set; }
        ...
        public List<Subplan> InputPlanPart { get; set; }
        ...
        protected bool lastGroundingReachedMaxDepth = false;
        public int MaxGroundingDepth { get; protected set; } = int.MaxValue;
        ...
        internal class CFGRule
        {
            ...
            internal IEnumerable<Subplan> GetGroundedSubplans(EarleyParser parser,
                ...
                Slot lastSlot, double lastEndIndex, List<IGroundable> usedSubtaskCompletingRules, CancellationToken cancellationToken,
                int groundingDepth = -1)
            {
                ...
                if (IsLeaf())
                {
                    return GetLeafSubplans(parser, startIndex,
                        correspondingQueueItem.IsActionInPrefix(parser), 
                        correspondingQueueItem.GetIndexOfCoveredAction(startIndex, parser), 
                        cancellationToken);
                }
                else
                {
                    return GetInternalNodeSubplans(parser, startIndex, SubtaskCompletingRules, maxAllowedCost, currentCost, lastSlot, lastEndIndex,
                        usedSubtaskCompletingRules, cancellationToken, groundingDepth);
                }
            }
            ...
            IEnumerable<Subplan> GetInternalNodeSubplans(EarleyParser parser, int startIndex,
                IEnumerable<IGroundable>[] subtaskCompletingRules, int maxAllowedCost, int currentCost,
Slot lastSlot, double lastEndIndex, List<IGroundable> usedSubtaskCompletingRules, 
CancellationToken cancellationToken, int groundingDepth)
            {
                if (groundingDepth == parser.MaxGroundingDepth)
                {
#if DEBUG
                    parser.ReachedMaximumGroundingDepth();
                ...
                foreach (var groundedSubtasks in
                    ...
                    cancellationToken, usedSubtaskCompletingRules))
                {
                ...
                List<IGroundable> usedSubtaskCompletingRules)
            {
                ...
                else
                {
                    ...
                    {
#if DEBUG
                        //Console.WriteLine($"SCR: {string.Join(", ",
                         //   usedSubtaskCompletingRules.Select(x => x.GetID()))}");
                        //debugWriter2.WriteLine($"SCR: {string.Join(", ",
                         //   usedSubtaskCompletingRules.Select(x => x.GetID()))}");
#endif
                                ...
                                Select(x => x.Type).ToList()))
                            {
                                ...
                                if (rule.GetCFGRule().CheckEmptyRuleConstraints(lastSlot))
                                {
                                    ...
                                    foreach (var result in
                                        ...
                                        cancellationToken, maxAllowedCost, nextCost, lastSlot,
                                        lastEndIndex, new(usedSubtaskCompletingRules) { rule}))
                                    {
                                        rule.ResetVariables();
                                        if (result != null)
                                        {
                        ...
                        else
                        {
                            foreach (var subplan in
                                rule.GetGroundedSubplans(parser, nextCost, lastSlot, 
                                lastEndIndex, new(usedSubtaskCompletingRules) { rule},
                                cancellationToken
                                ))
                            {
                                ...
                                foreach (var result in
                                    ...
                                    subtaskCompletingRules, cancellationToken, maxAllowedCost, 
                                    nextCost, nextSlot, subplan.EndIndex,
                                    new(usedSubtaskCompletingRules) { rule}))
                                {
                                    rule.ResetVariables();
                                    if (result != null)
                                    {
            ...
            private IEnumerable<Subplan> GetLeafSubplans(EarleyParser parser, int timelinePosition,
                bool isInPrefix, int coveredActionPositionInPrefix,
                CancellationToken cancellationToken) // returns all groundings
            {
                if (isInPrefix)
                {
                    var result = parser.GetActionFromPrefix(coveredActionPositionInPrefix);
                    if (result.TaskType.Name != (Subtasks[0] as PrimitiveTask).
...
#if DEBUG
                        debugWriter2.WriteLine($"GROUNDED ORIGINAL ACTION at index {parser.CurrentPlan.Count - 1}: " +
                            $"{result}");
                        debugWriter2.WriteLine($"\tfrom rule {this}");
#endif
                        yield return result;
                ...
                else
                {
                    ...
                    foreach (var tuple in parser.GenerateTasksFromPartiallyInstantiatedAction
                        (actionToGround, timelinePosition))
                    {
...
#if DEBUG
                            debugWriter2.WriteLine($"GROUNDED NEW ACTION at index {parser.CurrentPlan.Count - 1}: {tuple.Item2}");
                            debugWriter2.WriteLine($"\tfrom rule {this}");
#endif
            ...
            private int CoveredActionPosition(int timelinePosition)
            {
        ...
        private void ReachedMaximumGroundingDepth()
        {
            lastGroundingReachedMaxDepth = true;
        }
        ...
        internal Subplan GetActionFromPrefix(int timelinePosition)
        {
            return InputPlanPart[timelinePosition];
        }

        internal bool IsInPrefix(int timelinePosition)
        {
            return timelinePosition < InputPlanPart.Count;
        }
        ...
        {
            return GenerateTasksFromPartiallyInstantiatedAction(actionToGround, AllTaskTypes, 
                AllActionTypes, timelinePosition/* + 1*/,
        ...
        {
            IEnumerable<Subplan> GetGroundedSubplans(EarleyParser parser, int currentCost, 
                Slot lastSlot, double lastEndIndex, /*Action<int> minCostSetter,*/
List<IGroundable> usedSubtaskCompletingRules, CancellationToken cancellationToken);
            ...
            void ResetVariables();
            int GetIndexOfCoveredAction(int startIndex, EarleyParser parser);
        }

        internal class QueueItem : IGroundable
        {
            static int instancesCount = 0;
            readonly int ID = instancesCount++;
            ...
            internal void AddSubtaskCompletingRule(int subtaskIndex, QueueItem completingRule)
            {
                ...
                Debug.Assert(SubtaskCompletingRules[subtaskIndex].Count == 0 ||
                        SubtaskCompletingRules[subtaskIndex].Select(x =>
                    (x as QueueItem).IterationIndex).Distinct().Count() == 1);
            }
            ...
            public override bool Equals(object obj)
            {
                return obj != null && obj is QueueItem item &&
                       ...
                       CFGRule.Equals(item.CFGRule) && IterationIndexesEqual(item);

            }

            private bool IterationIndexesEqual(QueueItem item)
            {
                for (int i = 0; i < SubtaskCompletingRules.Length; i++)
                {
                    if (SubtaskCompletingRules[i].Count == 0 ||
                        item.SubtaskCompletingRules[i].Count == 0)
                    {
                        break;
                    }

                    if ((SubtaskCompletingRules[i].First() as QueueItem).IterationIndex
                        != (item.SubtaskCompletingRules[i].First() as QueueItem).IterationIndex)
                    {
                        return false;
                    }
                }
            ...
            public override string ToString()
            {
                return ID + ": " + CFGRule.ToString() + "; iteration:" + IterationIndex.ToString();
            }



            public IEnumerable<Subplan> GetGroundedSubplans(EarleyParser parser, int currentCost,
                    Slot lastSlot, double lastEndIndex, 
                    List<IGroundable> usedSubtaskCompletingRules, CancellationToken cancellationToken)
            {
                int maxAllowedCost = MaxAllowedCost(parser);
#if DEBUG
                debugWriter2.WriteLine($"Grounding node at index {parser.CurrentPlan.Count}: " + this.ToString());
                ...
                foreach (var subplan in CFGRule.GetGroundedSubplans(parser, this, IterationIndex, SubtaskCompletingRules,
                    maxAllowedCost, currentCost, lastSlot, lastEndIndex, usedSubtaskCompletingRules,
                    cancellationToken))
                {
#if DEBUG
                    groundingFound = true;
                    debugWriter2.WriteLine("\t" + string.Join(", ", parser.CurrentPlan));
                    debugWriter2.WriteLine($"GROUNDED at index {parser.CurrentPlan.Count - 1}: " + this.ToString() + ": " + subplan.ToString());
#endif
                    yield return subplan;
                }
                ...
                {
                    debugWriter2.WriteLine("\t" + string.Join(", ", parser.CurrentPlan));
                    debugWriter2.WriteLine($"Could not ground at index {parser.CurrentPlan.Count}: {this}");
            ...
            internal void CopySubtaskCompletingRulesFrom(QueueItem tableItem)
            {
                for (int i = 0; i < SubtaskCompletingRules.Length; i++)
                {
                    foreach (var rule in tableItem.SubtaskCompletingRules[i])
                    {
                        ...
                        Debug.Assert(SubtaskCompletingRules[i].Count == 0 ||
                        SubtaskCompletingRules[i].Select(x =>
                    (x as QueueItem).IterationIndex).Distinct().Count() == 1);
                    }
                }
            }
            ...
            internal int LastSubtaskCompletingRuleIterationIndex()
            {
                int lastCompletedSubtask = 0;
                while (lastCompletedSubtask < SubtaskCompletingRules.Length
                    && SubtaskCompletingRules[lastCompletedSubtask].Count > 0)
                {
                ...
                return (SubtaskCompletingRules[lastCompletedSubtask].First()
                    as QueueItem).IterationIndex;

            }

            internal bool EqualIterationIndexes(QueueItem existingItem)
            {
                for (int i = 0; i < SubtaskCompletingRules.Length; i++)
                {
                    if (SubtaskCompletingRules[i].Count == 0 || existingItem.SubtaskCompletingRules.Length == 0)
                    {
                        break;
                    }

                    if ((SubtaskCompletingRules[i].First() as QueueItem).IterationIndex !=
                        (existingItem.SubtaskCompletingRules[i].First() as QueueItem).IterationIndex)
                    {
                        return false;
                    }
            ...
            public int GetIndexOfCoveredAction(int finalTimelineIndex, EarleyParser parser)
            {
                return finalTimelineIndex;
            }
        }
            ...
            List<TaskType> allTaskTypes, CancellationToken cancellationToken, Task rootTask = null)
        {
            ...
            for (i = 0; i <= planPrefix.Count; i++)
            {
                ...
                OneIteration(i, cancellationToken, domains, table, allRulesExtended, queue, queueHashSet, out goalTask, 
                    i == InputPlanPart.Count, dummyStartingTask);
        ...
        protected Subplan RunEarleyParsingAsPlanRecognition(List<Action> planPrefix, List<ActionType> allActionTypes, List<Rule> allRules,
            List<TaskType> allTaskTypes, CancellationToken cancellationToken)
        {
            List<HashSet<Action>> domains = GetInitialDomains(planPrefix, 1, allActionTypes, 
                out HashSet<Action> allActions); // one additional domain to perform additional scanning
            List<HashSet<QueueItem>> table = new List<HashSet<QueueItem>>(planPrefix.Count + 2);      
            for (int j = 0; j <= planPrefix.Count + 1; j++)
            {
                table.Add(new HashSet<QueueItem>());
        ...
        protected Subplan TryExtractGoal(IGroundable queueItem, EarleyParser parser, 
            Slot dummyInitStateSlot,
            CancellationToken cancellationToken)
        {
#if DEBUG
            debugWriter2.WriteLine($"Trying to extract goal from {queueItem}");
#endif
            return queueItem.GetGroundedSubplans(parser, 0, dummyInitStateSlot, -1, new(), 
            ...
            int iterationIndex, int totalIterations, HashSet<QueueItem> queueHashSet, bool exceedingIterationLimitAllowed = false)
        {
            ...
            {
#if DEBUG
                Console.WriteLine($"\tSCANNER SUCCESS: iteration: {iterationIndex}, action: {action}");
#endif
            ...
            foreach (QueueItem tableItem in EnumerateTableItems(queueItem.IterationIndex, table, queueItem.CFGRule.MainTask))
            {
                    ...
                    NonConflictingInstantiations(nextTask.GetConstants(), queueItem.CFGRule.MainTask.GetConstants())) 
                {
                    ...
                    if (!table[iterationIndex].TryGetValue(newQI, out QueueItem existingItem) &&
                        (heuristicSelection || !queueHashSet.TryGetValue(newQI, out existingItem))
                        || (existingItem != null && 
                        existingItem.LastSubtaskCompletingRuleIterationIndex() != queueItem.IterationIndex
                        ) || !tableItem.EqualIterationIndexes(existingItem))
                    {
#if DEBUG
                        Console.WriteLine();
#endif
                        ...
                        && tableItem.EqualIterationIndexes(existingItem))
                    {
                        ...
                        existingItem.CopySubtaskCompletingRulesFrom(tableItem);
                    ...
                    {
#if DEBUG
                        Console.WriteLine(" (existing - queue)");
#endif
                    }
                }
            }
        ...
        protected virtual void InitializeQueue(Queue<QueueItem> queue, List<TaskType> allTaskTypes, out AbstractTask dummyStartingTask, 
            List<Rule> allRules,
        ...
        protected virtual List<HashSet<Action>> GetInitialDomains(List<Action> planPrefix, 
        ...
        internal bool Init(List<Term> plan, List<TaskType> allTaskTypes, 
            ...
            List<Term> initState)
        {
            ...
            foreach (Term a in plan)
            {
                ...
                if (i == 1)
                {
                    InputPlanPart[0].GetSlot(0).PosPreConditions = UnifyConditions(InputPlanPart[0].GetSlot(0).PosPreConditions, initState); // We assume init state has only positive conditions. If it has negative we can simply ignore them as negative just means not in the list of positive.
                    List<Term> c1 = RemoveConditions(InputPlanPart[0].GetSlot(0).PosPreConditions, InputPlanPart[0].GetSlot(0).NegPostConditions);
                    InputPlanPart[0].GetSlot(0).PosPostConditions = UnifyConditions(InputPlanPart[0].GetSlot(0).PosPostConditions, c1);
                    prevSlot = InputPlanPart[0].GetSlot(0);
                }
                else
                {
