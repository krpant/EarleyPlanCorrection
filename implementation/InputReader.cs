...
namespace PlanRecognitionNETF
{
    ...
    class InputReader
    {
        ...
        public List<Term> positiveStateChange = new(); // for plan repair
        public List<Term> negativeStateChange = new();
            ...
            List<Constant> allConstants)
        {
            ...
            if (forall == true) forall = false; //Last condition was for all now is the one it applies to.  
            bool isPositive = true;
        ...
        private TaskType CreateTaskType(string line) //From list of main tasks
        {
            ...
            string[] parts = line.Trim().Split(); //parts: makeTomatoSoup :parameters (?p - cookingPot)) //modified
            int index = line.IndexOf(";;"); //Removes everything after ;; which symbolizes comment
            if (index > 0)
            {
                line = line.Substring(0, index);
        ...
        public List<Term> ReadPlan(String fileName, List<ActionType> allActionTypes, List<Constant> allConstants, 
            ...
            bool readingPlanRepairProblem = false,
            bool readingPlanRepairProblemWithSuffix = false)
        { 
            System.IO.StreamReader file = new System.IO.StreamReader(fileName);
            ...
            while ((line = file.ReadLine()) != null)
            {
                ...
                {
                    if (( readingPlanRepairProblem && !readingPlanRepairProblemWithSuffix)
                        && a.ToLower() 
                        == EarleyPlanRepairer.STATE_CHANGE_ACTION_NAME.ToLower() + ")")
                    {
                        return myActions; // the prefix has been read
                    }
                    Term actionInstance = CreateActionInstance(a, allActionTypes, allConstants);
                    if (actionInstance != null)
                    {
                        ActionType aT = FindActionType(allActionTypes, actionInstance.Name, 
                            actionInstance.Variables.Count(), 
                            readingPlanRepairProblemWithSuffix);
                        if (!readingPlanRepairProblemWithSuffix || aT != null)
                        {
        ...
        internal static ActionType FindActionType(List<ActionType> allActionTypes,
            string name, int vars, bool readingPlanRepairProblemWithSuffix = false)
        {
            var result = allActionTypes.Where(x => x.ActionTerm.Name.Equals(name) && x.ActionTerm.Variables.Count() == vars).FirstOrDefault();

            if (result == null && readingPlanRepairProblemWithSuffix)
            {
                result = new ActionType()
                {
                    ActionTerm = new Term(EarleyPlanRepairer.STATE_CHANGE_ACTION_NAME,
        ...
        /// </summary>
        /// <param name="fileName"></param>
        public List<Term> ReadProblem(String fileName, List<ConstantType> allConstantTypes, 
            ref List<Constant> constants, bool readingPlanRepairProblem = false)
        {
            ...
            bool inStateChange = false; // for plan repair
            ...
            int lineNumber = 0; // for debug purposes
            ...
            while ((line = file.ReadLine()) != null)
            {
                ...
                if (!line.Trim().StartsWith(";;") && line.Trim().Length > 0)      { 
                    ...
                    if (line.Trim().Equals(")") && inInit == true)
                    {
                        if (!readingPlanRepairProblem)
                        {
                            return conditions;
                        }
                    }
                    if (line.Trim().Equals(")") && inStateChange == true)
                    {
                        return conditions;
                    }
                    ...
                    else if (inInit == true)
                    {
                        ...
                        {
                            Term c = CreateStateCondition(part, ref inputConstants,
                                constants);
                            if (c != null) conditions.Add(c);
                        }
                    }
                    else if (inStateChange == true)
                    {
                        string[] parts = Regex.Split(line, @"(?=\()");
                        bool negative = false;
                        foreach (string part in parts)
                        {
                            if (part.Trim() == "(not" || part.Trim() == "not")
                            {
                                negative = true;
                            }
                            ...
                            if (c != null)
                            {
                                if (negative)
                                {
                                    negativeStateChange.Add(c);
                                    negative = false;
                                }
                                else
                                {
                                    positiveStateChange.Add(c);
                                }
                            }
        ...
        private Term CreateStateCondition(string line, ref List<Constant> methodInfo, 
            List<Constant> allConstants)
        {
            ...
            if (tupleC != null)
            {
                ...
                if (tupleC.Item2 == false)
                {
                    Console.WriteLine("Negative initial condition. Currently ignoring.");
                }
                else
                {
                    Term c = tupleC.Item1;
