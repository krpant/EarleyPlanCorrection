...
namespace PlanRecognitionExtension
{
    internal class EntryPoint
    {
        ...
        enum SolverType
        {
            ...
            TIHTNPlannerInsertionOnTheFly, TIHTNPlannerInsertionAllAtOnce, PlanRepairAsPlanRecognition,
            PlanRepairAsPlanCorrection, PlanRepairAsPlanCorrectionWithSuffix, POEarleyV2
        }
        ...
        static void Main(string[] args)
        {
            ...
            {
                // DEBUG START
                string domain = "Blocksworld-GTOHP";
                string problem = "p04";
                string benchmarksDir = @"..\..\CORRECTION_Benchmarks";
                string domainS = @$"{benchmarksDir}\{domain}\domains\domain.hddl";
                string problemS = @$"{benchmarksDir}\{domain}\problems\{problem}.hddl";
                string planS = @$"{benchmarksDir}\{domain}\plan_correction\plans_with_added_actions\{problem}_add_1.txt";
                ...
                bool POActionInsertionAllowed = false;
                bool POActionDeletionAllowed = true;
                bool POAnytimeGoals = false;
                ...
                else if (args != null && args.Length >= 6)
                {
                    ...
                    if ((solverS == "repair_c" || solverS == "repair_c_suffix") && 
                        args.Length >= 7)
                    {
                        if (args[6] == "n")
                        {
                ...
                {
                    Console.WriteLine("Not enough arguments. Expects domain file path, problem file path, output file path, result csv file path, " +
                ...
                {
                    Console.WriteLine("Not enough arguments. Expects domain file path, problem file path, plan file path, result csv file path, \"o\" for the original recognizer, " +
                        "\"repair_pr\" for plan repair as plan recognition, \"p\" for recognizer with pruning, \"e\" for Earley plan recognition, \"po/po2\" for Earley recognition with partial observability, \"v\" for Earley verification, \"g\" for grounded Earley verification, or " +
                        "\"repair_pr/repair_c/repair_c_suffix\" for plan repair by plan recognition/plan correction with or without suffix, timeout in seconds + in case of partial observability 'action insertion allowed' (y/n), 'action deletion allowed' (y/n) and " +
                        "'anytime goals' (y/n).");
                    return;
                }
#endif
                ...
                List<Term> positiveStateChange = null;
                List<Term> negativeStateChange = null;
                    ...
                    try
                {
                    Read(planS, domainS, problemS, out plan, out allTaskTypes, out allActionTypes,
                        ...
                        out rules, out planPrefix, out initHtnTasks, out predicates, 
                        out positiveStateChange, out negativeStateChange,
                        solverS);
                }
                catch (Exception e)
                {
...
#endif
                {
                    ...
                    if (solverS.ToLower() == "o")
                    {
                        Console.WriteLine("Original recognizer started...");
                        solverType = SolverType.Original;
                    ...
                    else if (solverS.ToLower() == "p")
                    {
                        Console.WriteLine("Recognizer with Earley pruning started...");
                        solverType = SolverType.WithPruning;
                    ...
                    else if (solverS.ToLower() == "e")
                    {
                        Console.WriteLine("Earley recognizer started...");
                        solverType = SolverType.Earley;
                    ...
                    else if (solverS.ToLower() == "v")
                    {
                        Console.WriteLine("Earley verifier started...");
                        solverType = SolverType.EarleyVerifier;
                    ...
                    else if (solverS.ToLower() == "g")
                    {
                        Console.WriteLine("Grounded Earley verifier started...");
                        solverType = SolverType.GroundedEarleyVerifier;
                    ...
                    else if (solverS.ToLower() == "h")
                    {
                        Console.WriteLine("Earley recognizer with heuristic started...");
                        solverType = SolverType.HeuristicEarley;
                    ...
                    else if (solverS.ToLower() == "po2")
                    {
                        Console.WriteLine("Partially observable Earley (new version) recognizer started...");
                        solverType = SolverType.POEarleyV2;
                        isValid = RunPOEarleyV2(plan, allTaskTypes, allActionTypes, initialState, allConstantTypes, allConstants, emptyRules, rules, planPrefix,
                            POActionInsertionAllowed, POActionDeletionAllowed, POAnytimeGoals,
                            out finalRule, out finalSubtask, out addedActionsByIteration, cancellationToken, watch, out POAddedActions, out PODeletedActions, out POCorrectedPlan,
                            out POFoundGoalsWithTime);
                    }
                    else if (solverS.ToLower() == "pl")
                    {
                        Console.WriteLine("Earley planner started...");
                        solverType = SolverType.Planner;
                    ...
                    else if (solverS.ToLower() == "ti1")
                    {
                        Console.WriteLine("TIHTN planning started...");
                        solverType = SolverType.TIHTNPlannerInsertionOnTheFly;
                    ...
                    else if (solverS.ToLower() == "ti2")
                    {
                        Console.WriteLine("TIHTN planning started...");
                        solverType = SolverType.TIHTNPlannerInsertionAllAtOnce;
                        isValid = RunTIHTNPlanner(classicalPlanner, allTaskTypes,
                            allActionTypes, initialState, allConstantTypes, allConstants,
                            emptyRules, rules, predicates,
                            POAnytimeGoals, out finalRule, out finalSubtask,
                            cancellationToken, watch, out POAddedActions,
                            out POCorrectedPlan,
                            out POFoundGoalsWithTime,
                            initHtnTasks, TIHTNPlanner.TaskInsertionMode.InsertAllAtOnce, returnFirstFoundSolution);
                    }
                    else if (solverS.ToLower() == "repair_pr")
                    {
                        Console.WriteLine("HTN plan repair as plan recognition started...");
                        solverType = SolverType.PlanRepairAsPlanRecognition;
                        isValid = RunRepairPR(plan, allTaskTypes, allActionTypes,
                            initialState, allConstantTypes, allConstants, emptyRules,
                            rules, planPrefix, out finalRule, out finalSubtask,
                            out addedActionsByIteration, initHtnTasks,
                            predicates,
                            rules, positiveStateChange, negativeStateChange,
                            cancellationToken);

                    }
                    else if (solverS.ToLower() == "repair_c")
                    {
                        Console.WriteLine("HTN plan repair as plan correction started...");
                        solverType = SolverType.PlanRepairAsPlanCorrection;
                        isValid = RunRepairC(plan, allTaskTypes, allActionTypes,
                            initialState, allConstantTypes, allConstants, emptyRules,
                            rules, planPrefix, out finalRule, out finalSubtask,
                            out addedActionsByIteration, initHtnTasks,
                            predicates,
                            rules, positiveStateChange, negativeStateChange,
                            cancellationToken, POAnytimeGoals, watch,
                            out POFoundGoalsWithTime, out POAddedActions,
                            out PODeletedActions, out POCorrectedPlan);
                    }
                    else if (solverS.ToLower() == "repair_c_suffix")
                    {
                        Console.WriteLine("HTN plan repair as plan correction with suffix started...");
                        solverType = SolverType.PlanRepairAsPlanCorrectionWithSuffix;
                        isValid = RunRepairCSuffix(plan, allTaskTypes, allActionTypes,
                            initialState, allConstantTypes, allConstants, emptyRules,
                            rules, planPrefix, out finalRule, out finalSubtask,
                            out addedActionsByIteration, initHtnTasks,
                            predicates,
                ...
                if (solverType != SolverType.POEarley && solverType != SolverType.EarleyVerifier
                    && solverType != SolverType.GroundedEarleyVerifier && solverType != SolverType.Planner
                    && solverType != SolverType.TIHTNPlannerInsertionOnTheFly 
                    && solverType != SolverType.TIHTNPlannerInsertionAllAtOnce
                    && solverType != SolverType.PlanRepairAsPlanCorrectionWithSuffix
                    && solverType != SolverType.PlanRepairAsPlanCorrection)
                {
                    WriteStatsToCsvFile(planS, domainS, problemS, resultS, prefixLength, finalSubtask, solverType, exitCode, elapsedMs, 
                        addedActionsByIteration, exception);
                }
                else if (solverType == SolverType.POEarley || solverType == SolverType.POEarleyV2 || solverType == SolverType.Planner
                    || solverType == SolverType.PlanRepairAsPlanCorrection ||
                    solverType == SolverType.PlanRepairAsPlanCorrectionWithSuffix)
                {
                    WritePOCsvFile(solverType, planS, domainS, problemS, resultS, 
                        finalSubtask, exitCode, elapsedMs, POAddedActions, PODeletedActions, 
                        POCorrectedPlan, POFoundGoalsWithTime, POActionInsertionAllowed, 
                        POActionDeletionAllowed,
                        exception);
                }
                else if (solverType == SolverType.EarleyVerifier || solverType == SolverType.GroundedEarleyVerifier)
                {
                    WriteVerificationStatsToCsvFile(planS, domainS, problemS, resultS, prefixLength, finalSubtask, solverType, 
                        exitCode, elapsedMs, addedActionsByIteration, exception);
                }
                else if (solverType == SolverType.TIHTNPlannerInsertionOnTheFly 
                    || solverType == SolverType.TIHTNPlannerInsertionAllAtOnce)
                {
                    WriteTIHTNcsvFile(solverType, domainS, problemS, resultS, exitCode, elapsedMs, POCorrectedPlan, POFoundGoalsWithTime,
                        exception);
                }

                WriteConsoleOutput(planS, domainS, problemS, isValid, elapsedMs, finalSubtask, solverType, exitCode, exception);
        ...
        private static bool RunPOEarleyV2(List<Term> plan, List<TaskType> allTaskTypes, List<ActionType> allActionTypes, List<Term> initialState, List<ConstantType> allConstantTypes, List<Constant> allConstants,
            ...
            CancellationToken cancellationToken, Stopwatch watch, out int addedActions, out int deletedActions, out string planString, out string foundGoalsWithTime)
        {
            ...
            NewPlanCorrector planR = new NewPlanCorrector(allTaskTypes, allActionTypes, allConstants, allConstantTypes,
        ...
        private static bool RunRepairCSuffix(List<Term> plan, List<TaskType> allTaskTypes,
            List<ActionType> allActionTypes, List<Term> initialState,
            List<ConstantType> allConstantTypes, List<Constant> allConstants,
            List<Rule> emptyRules, List<Rule> rules, List<Action> planPrefix,
            out Rule finalRule, out Subplan finalSubtask,
            out List<int> addedActionsByIteration, List<Task> initHtnTasks,
            List<Term> allPredicates, List<Rule> allRules,
            List<Term> positiveStateChanges, List<Term> negativeStateChanges,
            CancellationToken cancellationToken, bool anytimeGoals, Stopwatch watch,
            out string foundGoalsWithTime,
            out int addedActions, out int deletedActions, out string planString)
        {
            PlanRecognizerExtended.AddTaskToActionRules(allActionTypes, rules, allTaskTypes);
            List<Rule> allRulesWithEmptyRules = new List<Rule>(rules);
            allRulesWithEmptyRules.AddRange(emptyRules);
            PlanCorrectionRepairer repairer = new(allTaskTypes, allActionTypes, allConstants,
                allConstantTypes, initialState, initHtnTasks, allPredicates, allRules,
                anytimeGoals);
            bool result = repairer.RepairPlanWithSuffix(plan, planPrefix, 
                positiveStateChanges,
                negativeStateChanges, out finalRule, out finalSubtask, 
                out addedActionsByIteration,
                out List<ActionSubplan> foundPlan, watch, out foundGoalsWithTime, 
                cancellationToken);
            if (foundPlan != null)
            {
                addedActions = foundPlan.Count;
                planString = string.Join(", ", foundPlan);
            }
            else
            {
                addedActions = 0;
            ...
            WritePOSolution(finalSubtask, foundPlan, repairer, true, true, anytimeGoals, 
                out addedActions, out deletedActions, out planString);
            return result;
        }

        private static bool RunRepairC(List<Term> plan, List<TaskType> allTaskTypes,
            List<ActionType> allActionTypes, List<Term> initialState,
            List<ConstantType> allConstantTypes, List<Constant> allConstants,
            List<Rule> emptyRules, List<Rule> rules, List<Action> planPrefix,
            out Rule finalRule, out Subplan finalSubtask,
            ...
            out int addedActions, out int deletedActions, out string planString)
        {
            ...
            PlanCorrectionRepairer repairer = new(allTaskTypes, allActionTypes, allConstants,
                allConstantTypes, initialState, initHtnTasks, allPredicates, allRules,
                anytimeGoals);
            bool result = repairer.RepairPlanWithoutSuffix(plan, planPrefix, positiveStateChanges,
                negativeStateChanges, out finalRule, out finalSubtask, out addedActionsByIteration,
                out List<ActionSubplan> foundPlan, watch, out foundGoalsWithTime, cancellationToken);
            if (foundPlan != null)
            {
                addedActions = foundPlan.Count;
                planString = string.Join(", ", foundPlan);
            ...
            WritePOSolution(finalSubtask, foundPlan, repairer, true, false, anytimeGoals,
                out addedActions, out deletedActions, out planString);
            
            return result;
        }

        private static bool RunRepairPR(List<Term> plan, List<TaskType> allTaskTypes, 
            List<ActionType> allActionTypes, List<Term> initialState, 
            List<ConstantType> allConstantTypes, List<Constant> allConstants, 
            List<Rule> emptyRules, List<Rule> rules, List<Action> planPrefix, 
            out Rule finalRule, out Subplan finalSubtask, 
            ...
            CancellationToken cancellationToken)
        {
            ...
            List<Rule> allRulesWithEmptyRules = new List<Rule>(rules);
            allRulesWithEmptyRules.AddRange(emptyRules);
            EarleyPlanRepairer repairer = new(allTaskTypes, allActionTypes, allConstants,
                allConstantTypes, initialState, initHtnTasks, allPredicates, allRules);
            return repairer.RepairPlan(plan, planPrefix, positiveStateChanges,
                negativeStateChanges, out finalRule, out finalSubtask, out addedActionsByIteration,
                cancellationToken);

        }

        private static void WriteTIHTNcsvFile(SolverType recognizerType, string domainS, string problemS, string resultS, 
            ...
            out string planString, string hSpace = "", bool writeHeader = true)
        {
            ...
            if (finalSubtask != null)
            {
                if (writeHeader)
                {
                    Console.WriteLine("NUMBER OF FLAWS: " + parser.ComputeNumberOfFlaws(foundPlan));
                ...
                for (int indexInSolution = 0; indexInSolution < foundPlan.Count; indexInSolution++)
                {
                    ...
                    if (a.IsInPlan)
                    {
                        while (!a.Subplan.Equals(parser.InputPlanPart[indexInPlan]))
                        {
                            line = $"{hSpace} - {indexInPlan}: {parser.InputPlanPart[indexInPlan].ActionString(0)}";
                            AddLineToPlanString(ref planString, line);
                            Console.WriteLine(line);
                            deletedActions++;
                            indexInPlan++;
                ...
                while (indexInPlan < parser.InputPlanPart.Count)
                {
                    line = $"{hSpace} - {indexInPlan}: {parser.InputPlanPart[indexInPlan].ActionString(0)}";
                    AddLineToPlanString(ref planString, line);
                    Console.WriteLine(line);
        ...
        private static void WriteConsoleOutput(string planS, string domainS, string problemS, bool isValid, long elapsedMs, 
            Subplan finalSubtask, SolverType recognizerType, ExitCode exitCode, Exception exception = null)
        {
            ...
            {
                case SolverType.WithPruning: Console.WriteLine("Plan recognition with pruning (Earley CFG parser):");
                    break;
                case SolverType.Original: Console.WriteLine("Plan recognition without pruning:");
                    break;
                case SolverType.EarleyVerifier: Console.WriteLine("Earley plan verification:");
                    break;
                case SolverType.Planner: Console.WriteLine("Earley planner:");
                    break;
                case SolverType.TIHTNPlannerInsertionAllAtOnce: Console.WriteLine("TITHN planner - insertion all at once:");
                    break;
                case SolverType.TIHTNPlannerInsertionOnTheFly: Console.WriteLine("TIHTN planner - insertion on the fly");
                    break;
            ...
            switch (exitCode)
            {
                case ExitCode.Solved:
                    if (isValid)
                    {
                        ...
                        if (recognizerType != SolverType.TIHTNPlannerInsertionOnTheFly
                            && recognizerType != SolverType.TIHTNPlannerInsertionAllAtOnce)
                        {
        ...
        static void WritePOCsvFile(SolverType recognizerType, string planS, string domainS, string problemS, string resultS,
            ...
            string foundGoalsWithTime, bool actionInsertionAllowed, bool actionDeletionAllowed, Exception exception = null)
        {
            ...
            using (StreamWriter csvWriter = new StreamWriter(resultS, true))
            {
                ...
                if (recognizerType != SolverType.Planner)
                {
        ...
        private static void WriteVerificationStatsToCsvFile(string planS, string domainS, string problemS, string resultS,
            int planLength, Subplan finalSubtask, SolverType recognizerType, ExitCode exitCode, long time, List<int> addedActions, Exception exception = null)
        {
            bool writeHeader = !File.Exists(resultS);
            using (StreamWriter csvWriter = new StreamWriter(resultS, true))
            {
        ...
        private static void WriteStatsToCsvFile(string planS, string domainS, string problemS, string resultS,
            int prefixLength, Subplan finalSubtask, SolverType recognizerType, ExitCode exitCode, long time, List<int> addedActions, Exception exception = null)
        {
            bool writeHeader = !File.Exists(resultS);
            using (StreamWriter csvWriter = new StreamWriter(resultS, true))
            {
...
out List<Term> positiveStateChange, out List<Term> negativeStateChange, string solverType)
        {
            InputReader reader = new InputReader();
            reader.ReadDomain(domainS);
            allActionTypes = reader.globalActions;
            allConstantTypes = reader.allConstantTypes;
            if (solverType == "repair_pr" || solverType == "repair_c" 
                || solverType == "repair_c_suffix")
            {
                initialState = reader.ReadProblem(problemS, allConstantTypes,
                    ref reader.allConstants, true);
            }
            else
            {
                initialState = reader.ReadProblem(problemS, allConstantTypes, 
                    ref reader.allConstants);
            }
            
            emptyRules = reader.emptyRules;
            allConstants = reader.allConstants;
            predicates = reader.allPredicates;
            positiveStateChange = reader.positiveStateChange;
            negativeStateChange = reader.negativeStateChange;
                        
            if (solverType != "pl" && !solverType.StartsWith("ti"))
            {
                if (solverType == "repair_pr" || solverType == "repair_c")
                {
                ...
                else if (solverType == "repair_c_suffix")
                {
                    reader.ReadPlan(planS, allActionTypes, allConstants, out planPrefix,
                        true, true);
                }
